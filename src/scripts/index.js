...
cards.forEach(item => {
    ...
    universe.textContent = `Вселенная: ${item.universe}`;

    const alterego = document.createElement('p');
    alterego.textContent = `Альтер эго: ${item.alterego}`;

    const occupation = document.createElement('p');
    occupation.textContent = `Род деятельности: ${item.occupation}`;

    const friends = document.createElement('p');
    friends.textContent = `Друзья: ${item.friends}`;
